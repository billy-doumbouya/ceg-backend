...
router.post(
  "/projects",
  requireAuth,
  uploaders.projects.single("image"),
  projectsCtrl.create,
);
router.put(
  "/projects/:id",
...
router.post(
  "/gallery/categories",
  requireAuth,
  uploaders.gallery.single("cover"),
  galleryCtrl.createCategory,
);
router.put(
  "/gallery/categories/:id",
  requireAuth,
  uploaders.gallery.single("cover"),
  galleryCtrl.updateCategory,
);
router.delete(
  "/gallery/categories/:id",
  requireAuth,
  galleryCtrl.deleteCategory,
);
...
router.put(
  "/gallery/images/:id",
  requireAuth,
  uploaders.gallery.single("image"),
  galleryCtrl.updateGalleryImage,
);
router.post(
  "/gallery/images",
  requireAuth,
  uploaders.gallery.array("images", 50),
  galleryCtrl.uploadImages,
);
router.delete(
  "/gallery/images/:id",
  requireAuth,
  galleryCtrl.deleteGalleryImage,
);
...
router.post(
  "/partners",
  requireAuth,
  uploaders.partners.single("logo"),
  partnersCtrl.create,
);
router.put(
  "/partners/:id",
...
router.post(
  "/testimonials",
  requireAuth,
  uploaders.testimonials.single("avatar"),
  testimonialsCtrl.create,
);
router.put(
  "/testimonials/:id",
  requireAuth,
  uploaders.testimonials.single("avatar"),
  testimonialsCtrl.update,
);
...
router.get("/contact/admin", requireAuth, contactCtrl.getAllAdmin);
router.patch("/contact/:id/read", requireAuth, contactCtrl.markAsRead);
router.delete("/contact/:id", requireAuth, contactCtrl.remove);

// ─── DONATIONS ───────────────────────────────────────────────────────────────
router.post(
  "/donations",
  [
    body("donorName").notEmpty().withMessage("Le nom est requis"),
    body("donorEmail").isEmail().withMessage("Email invalide"),
    body("amount").isNumeric().withMessage("Montant invalide"),
    body("transactionId").notEmpty().withMessage("ID de transaction requis"),
  ],
  validate,
  donationsCtrl.create,
);
router.get("/donations/admin", requireAuth, donationsCtrl.getAllAdmin);
router.patch("/donations/:id/read", requireAuth, donationsCtrl.markAsRead);
router.delete("/donations/:id", requireAuth, donationsCtrl.remove);

// ─── BACKGROUNDS ─────────────────────────────────────────────────────────────
router.get("/backgrounds", backgroundsCtrl.getAll);
router.get("/backgrounds/:pageKey", backgroundsCtrl.getOne);
router.put(
  "/backgrounds/:pageKey",
  requireAuth,
  uploaders.backgrounds.single("image"),
