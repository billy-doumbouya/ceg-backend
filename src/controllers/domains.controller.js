...
const create = asyncHandler(async (req, res) => {
  const body = { ...req.body };
  if (body.activities)
    body.activities = Array.isArray(body.activities)
      ? body.activities
      : JSON.parse(body.activities);
...
const update = asyncHandler(async (req, res) => {
  ...
  if (!item)
    return res
      .status(404)
      .json({ success: false, message: "Domaine non trouvé" });
  const fields = [
    "title",
    "shortTitle",
    "icon",
  ...
  if (req.body.activities !== undefined)
    item.activities = Array.isArray(req.body.activities)
      ? req.body.activities
