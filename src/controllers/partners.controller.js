...
const update = asyncHandler(async (req, res) => {
  ...
  if (req.file) {
    ...
    partner.logo = { url: req.file.path, publicId: req.file.filename };
  }
  const fields = ["name", "fullName", "logoText", "logoColor", "category", "description", "website", "partnership", "since", "order", "isPublished"];
  fields.forEach((f) => { if (req.body[f] !== undefined) partner[f] = req.body[f]; });
  if (req.body.domains) partner.domains = Array.isArray(req.body.domains) ? req.body.domains : JSON.parse(req.body.domains);
  await partner.save();
